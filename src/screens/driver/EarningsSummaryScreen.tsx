  ...
  const renderItem = ({ item }: { item: Earnings }) => (
    <View style={styles.row}>
      <View style={styles.dateCol}>
        <Text style={styles.dayText}>{new Date(item.date).toLocaleDateString('en', { weekday: 'short' })}</Text>
        <Text style={styles.dateText}>{new Date(item.date).toLocaleDateString('en', { month: 'short', day: 'numeric' })}</Text>
      </View>
      <Text style={styles.tripsText}>{item.trips} trips</Text>
        ...
        ListHeaderComponent={
          <View>
            <View style={styles.headerRow}>
              ...
              <Text style={[T.h2, styles.headerTitle]}>Earnings</Text>
            ...
            <View style={styles.statsRow}>
              ...
              <StatCard label="Trips" value={String(trips)} />
              <StatCard label="Avg/Trip" value={`$${avg}`} />
            </View>

            <Text style={[T.xs, { marginTop: Spacing.s4, marginBottom: Spacing.s2 }]}>History</Text>
          </View>
        }
        ListFooterComponent={
          <View style={{ marginTop: Spacing.s4, paddingBottom: insets.bottom + 24 }}>
            <CTA
              color="orange"
              onPress={() => Alert.alert('Payout', `Payout of $${total} requested. Funds arrive in 2 business days.`)}
            >
              <Text style={{ fontFamily: Fonts.bodyBold, fontSize: 16, color: '#1A0700', fontWeight: '700' }}>
                Request Payout ${total}
              </Text>
            </CTA>
          </View>
        }
